...
def menu(camera):
    ...
    imgui.set_next_window_size(160, 350)
...
def photo_button():
    imgui.set_next_window_size(120, 240)
    imgui.set_next_window_position(1160, 0)
    imgui.begin(
        "Menu",
        flags=imgui.WINDOW_NO_MOVE | imgui.WINDOW_NO_RESIZE
    )
    if imgui.button("Screenshot", width=100, height=100) == True:
        imgui.end()
        return 'Screenshot'
    if imgui.button("Constellation", width=100, height=100) == True:
        imgui.end()
